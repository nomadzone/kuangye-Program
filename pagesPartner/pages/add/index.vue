	...
	<view class="partner-add-page">
		...
		<form style="display:block;zIndex:11" @submit="handleSubmit">
			<view class="release-form-box">
				<textarea class="content-textarea" name="describe" auto-height placeholder="有没有人浅咖一下？"
					:maxlength="30" />
				<Upload :limit="9" @upload="doUploadPartnerImg" />

				<view class="location-row" @tap="handleSelectLocation()">
					<image class="map-icon" src="../../static/images/partner-meet-location.svg"></image>
					<view class="location">
						<text
							class="location-text">{{location.longitude && location.latitude ? location.address : '面基地点（选填）'}}</text>
				...
				<view class="location-row" @tap="handleOpenMeetingTime()">
					...
					<view class="location">
				<text
					class="location-text">{{meetingTime ? meetingTime : '面基时间（选填）'}}</text>
						<image class="location-select-icon" src="../../static/images/select-more.svg"></image>
					</view>
				</view>
			</view>
			...
			<view class="release-form-box">
				...
				<view class="location-row">
					...
					<view class="location">
						<input class="location-text" name="number" placeholder="输入手机号（选填）"/>
					</view>
				</view>
		...
		<uv-picker title="期望时间（选填）" confirmText="保存" ref="meetingTimeRef" :columns="timeList"
			@confirm="handleConfirmMeetingTime"></uv-picker>
	</view>
</template>
